...
public class collisionDestroyTrigger : MonoBehaviour
{
    ...
    //Defining
    private string bulletObject = "Pew";
    private string shotgunBulletObject = "Shotgun";
    private string shieldObject = "Shield";
    private string massiveProjectileObject = "MP";
    ...
    void OnCollisionEnter(Collision col)
    {
        ...
        for (int i = sending.cloneAddingPew; i > -ammo; i--)
        {
            if (col.gameObject.name == $"Pellet{i}")
            {
                ...
                if (col.gameObject.tag == bulletObject)
                {
                    ...
                    {
                        //Destroys pellet
                        Invoke("leaveOurMindsAlone",0.1f);
                        //Makes animation
                ...
                else if (col.gameObject.tag == shotgunBulletObject)
                {
                    //Destroy object it is colliding with
                    //Destroys object it is attached to (asteroid)
                    if (scaleOfAsteroid >= asteroidSizeToSpawnMore)
                ...
                else if (col.gameObject.tag == shieldObject)
                {
                  //Makes animation
                  Explode(col.transform);
                  //Spawns a token
                ...
                else if (col.gameObject.tag == massiveProjectileObject)
                {
                    //Makes animation
                    Explode(col.transform);
                    //Spawns a token
